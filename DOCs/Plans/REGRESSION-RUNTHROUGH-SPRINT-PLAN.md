...
| ------ | ----------- | ---- | --- |
| §1 | Skip-signal contract: loud, two-category skip markers + helpers | «#15» | 1 |
| §2 | Reclassify preconditions (baseline-guaranteed vs hardware-limit) | «#16» | 2 |
| §3 | Runner-enforced baseline (audit→format suite #0, scratch-gated) | «#17» | 3 |
| §4 | Runner skip-visibility + fail-on-skip + Skipped column | «#18» | 4 |
| §5 | Per-test starting-condition discipline — fix 3 violations | «#19» | 5 |
| §5 | Per-test starting-condition discipline — systemic retrofit (~13 suites) | «#20» | 6 |
| §5/§7 | Adversarial harness (dirty-card seeder + shuffle/standalone modes) | «#21» | 7 |
| §6 | Docs: contract extract, CLAUDE.md reporting, cross-refs, handoff pointer | «#22» | 8 |
| §7 | End-to-end determinism certification (native hardware; subsumes old #13) | «#23» | 9 |

Carried-over (deprioritized, not part of this sprint's seq): «#14» root-cause 462fd28 Heisenbug
(deferred — does not affect implementation); «#13» fixture-discipline native cert (subsumed by «#23»).
