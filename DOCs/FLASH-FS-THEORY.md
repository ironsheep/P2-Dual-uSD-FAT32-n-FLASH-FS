# Flash Filesystem - Theory of Operations

> **Context**: This document describes the Flash subsystem within the unified dual-FS driver (`dual_sd_fat32_flash_fs.spin2`). It is the authoritative reference for Flash internals including block formats, lifecycle bits, translation tables, the mount process, and wear leveling. For the overall dual-device architecture, see [DUAL-DRIVER-THEORY.md](./DUAL-DRIVER-THEORY.md).
...
- Directory emulation via path-segmented filenames


## Initial build - Constants

Key Constants in `dual_sd_fat32_flash_fs.spin2` describe:

- **FIRST_BLOCK** - The starting block to be used within the Flash chip. By default the value is chosen to allow for the first 512KB to be reserved for the boot-loaded code for the P2. (Default: $080)
- **LAST_BLOCK** - The address of the last block to be allocated to the filesystem. This effectively tells us the amount of space which is allocated to this filesystem (Default: $FFF)
- **MAX_OPEN_FILES** - a 4K buffer is allocated to each file we could have open at the same time as another. This is set to 2 so we can at least copy one file to another. (Default: 2)
- **BLOCK_SIZE** - fixed at 4KB, this is the smallest erasable amount of space within the SPI FLASH chip we use. (Default $1000)

**NOTE**: *The SPI signal lines are shared between the SD card and the Flash chip on P2 Edge Modules. MOSI, MISO, and SCK are shared; each device has its own CS line. The unified driver manages bus arbitration between the two devices. See [SPI Bus State Analysis](./Analysis/SPI-BUS-STATE-ANALYSIS.md) for details.*
...
**Flash Write** - a flash write only changes 1 bits to 0's. If a bit is 0 it can not be changed until the block containing it is erased at which time the bit becomes 1 once again.
...
A file is a one-way linked list. From the study of the block structures above, we see that the file contains control structures and data. The last block of the file always contains the End-Pointer which is the address of the next byte to be written or the value of $FFC which points to the CRC which means there is no more room to write in this block. In this condition (end pointer is $FFC) a next write to this file will first allocate a new block adding more space in which to write. The End pointer in this new last block is set to the first byte of data space in the new block. The former last block is set to point to the new last block and its type is set to body/more instead of body/last.

The file's control data also contains logical next pointers in each block except the last block. These next pointers contain the block ID of next block that is part of this file. We use block IDs instead of block addresses so the blocks containing the ID can be replaced with another block having the same ID when the file content changes but the blocks pointing to this ID don't have to change.
...
| <pre>append [%"r+", %"R+"]</pre> | YES | Read Extended from, Write Extended to an **existing file**, extending the existing file by writing records at end, and/or modifying the middle of the file by seeking to a location then overwriting a record at that location within the existing file. <BR>Supports direct access via seek()|
...
- A file starts out as a **Head/Last block** and remains so while it is <= 3,956 bytes.
...
- **Case #1**: In the case of appending to a longer file (a file which is 2 or more blocks) then the append consists of writing to the **Body/Last block** until we attempt to write the byte just past the end of this block. At this time we allocate a new **Body/Last block** to contain this new byte and rewrite the prior **Body/Last block** as a **Body/More block**.

The less frequent case is when we append to a file that contains less than 3,956 bytes:

- **Case #2**: In the case of a small file we write into the (**Head/Last block**) until we fill it. The growth of this file follows the sequence shown in "Writing to a File" (above.)
...
Internally this is all handled very simply. The user seeks to the location in the file. The seek locates the block containing the byte at the seek address which is then loaded into our block buffer for the file handle. The next writes then are written to this block in the handle buffer. If we attempt to write beyond the end of this block (or we seek to a location outside of this block) then this block is first replaced and activated and the new block is loaded into the buffer either where we are continuing the current write or are ready for the next write.
...
When a file is opened for sequential writing all the writes are accumulated into what we call a "commit chain". As a commit chain block fills it is written to the filesystem. Upon close the last block is written to the filesystem. Lastly, the commit chain head block is activated. If there was a prior file (we are overwriting or appending) then the commit head block will supersede the prior file`s block and the prior block(s) will be deleted. If the power fails just before the head is activated, the block will be discovered (block of same ID but with newer lifecycle) at mount and the older block will be canceled during the mount.
...
Under the covers this circular behavior affects reads and writes. When **opened for read as circular** the first data returned will be from the front of the file unless the file has reached the max size. If it has then the first data returned will instead be the data at the (file length - max size) offset (The front of your circular buffer.)

When the file is **opened for write as circular** after the writes are completed and the file is being closed or flushed then the entire write commit chain is written to flash. After this file is complete on flash then the file is froncated (Front Truncated) removing any/all leading blocks leaving enough blocks to contain the **max\_file\_length** bytes at the tail of the file blocks. If any blocks are removed during Froncation this means the file head block effectively moves to be the new leading block of the file blocks not removed during this Fruncate process.
...
In this mode, we think of the file as containing records. Open the file, seek to a location, read the existing record, make a change in it, then seek back to the record location and write the newly modified record. We can do this anywhere within the existing file or we can append records to the file.
...
When the filesystem is first mounted the following 3 tables are zeroed and then populated by `mount(DEV_FLASH)` as it scans all the blocks in the space allocated to the filesystem. The tables are:
...
Each 2-bit variable contains a block state enum value [sFREE, sTEMP, sHEAD, sBODY].
...
For each open file (handle) the driver maintains the following information:

| Name |Allocation | Purpose | Notes
| --- | --- | --- | --- |
| | **File State tracking**
| hStatus | BYTE    0[MAX_OPEN_FILES] | handle: status [H\_READ, H\_WRITE, H\_REPLACE, 0 not in use]| 1 byte
| hHeadBlockID | WORD    0[MAX_OPEN_FILES] | handle: first blockID of file chain | 2 bytes
| hEndPtr | WORD    0[MAX_OPEN_FILES] | handle: pointer to next write byte in block (or $FFC if block full) | 2 bytes
| hFilename | BYTE    0[MAX_OPEN_FILES * FILENAME\_SIZE] | handle: 127+1 byte buffer for filename | 128 bytes
| hBlockBuff | BYTE    0[MAX_OPEN_FILES * BLOCK\_SIZE] | handle: 4KB buffer for file data | <pre>4096 bytes</pre>
| hModified | BYTE    0[MAX_OPEN_FILES] | T/F where T means the buffer has been modified and needs to be written to flash | 1 byte
| | **Commit-chain tracking**
| hChainBlockID | WORD    0[MAX_OPEN_FILES] | handle: first blockID of commit chain | 2 bytes
| hChainBlockAddr | WORD    0[MAX_OPEN_FILES] | handle: first block_address of commit chain | 2 bytes
| hChainLifeCycle | BYTE    0[MAX_OPEN_FILES] | handle: first block lifecycle (cycle value for replacement first block of commit chain) | 1 byte
| | **Special Mode tracking**<br>**(Seeking, Circular files)**
| hSeekPtr | WORD    NOT\_ENABLED[MAX_OPEN_FILES] | handle: seek address within active block | 2 bytes|
| hSeekFileOffsetPtr | LONG    0[MAX_OPEN_FILES] | handle: logical seek address within entire file | 4 bytes
| hCircularLength | WORD    NOT\_ENABLED[MAX_OPEN_FILES] | handle: circular buffer length in max blocks | 2 bytes
| | | | **4,243 bytes / handle**


## The Format Process [F]

Formatting the Flash is a simple process. The first byte of every block allocated to the flash filesystem is read and if the lifecycle bits indicate the block is active [%011, %101, or %011] then the block is "canceled." That is the lifecycle bits are set to %000 and the byte is written to flash. Once this is complete all blocks will be seen as not in use when the mount occurs.
...
The end product of this search is that all file locations are now known in addition to which blocks belong to files. We also know if any blocks are somehow active but no longer belong to any files.

### [M3] Clearing blocks that no longer read correctly

The final pass then clears these somehow damaged blocks by canceling each of them effectively marking each of them as free/not in use. This "clearing" is not a normal case. It only happens when the flash memory has somehow changed state (or aged). In the case of blocks being found during the mount effort and then canceled an error code **E\_BAD\_BLOCKS_REMOVED** is returned from the mount call. This is to allow an application to be able to detect if files are somehow being lost to the aging effects of the flash chip.
...
*Flash Filesystem Theory of Operations for the P2 Dual uSD + FLASH Filesystem project*
